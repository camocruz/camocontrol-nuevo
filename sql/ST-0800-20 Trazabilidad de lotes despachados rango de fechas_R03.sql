...
((select f0421_id_despacho as id_desp, f0420_id_producto as id_prod
	...
	) -- Estos son todos los items pistoleados
union
  (select f0850_id_despacho, f0851_id_item
	...
	) -- Estos son todos los items remisionados
),
-- Esta tabla unifica la informaion de trazabilidad requerida por cada codigo de etiqueta
tb_info_trazable as 
(select distinct f0420_lote as lote, to_char(f0420_fecha_vence, 'YYYY-MM-DD') as fecha_vence, 
...
-- genero la consulta usando las tablas anteriores como base.
select f0800_id_despacho as id_pdd, to_char(f0800_fecha_despacho, 'YYYY-MM-DD') as f_despacho, 
	...
	from tb_union_items_desp
		...
		join camocontrol.tb0800_despachos_comercial
		  on id_desp = f0800_id_despacho
		left join camocontrol.tb0300_items
         on f0300_id_item = id_prod
...
WHERE f0800_id_cia = '00000001' and f0800_anulado = 'N' 
       ...
       and f0800_fecha_despacho BETWEEN '2023-04-01' AND '2024-01-08'
group by id_pdd, f_despacho,id_item, referencia, item, lote, f_vence,
         cliente, nit, ciudad, departamento
