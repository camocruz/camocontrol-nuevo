...
from $df001$.tb0600_acciones
     ...
     left join $df001$.tb0100_estructura_mantenimiento as maquina_rp
          on f0402_id_maquina = maquina_rp.f0100_id_estructura
     left join $df001$.tb0609_modos_efectos_falla as tb_modo_falla
          on tb_modo_falla.f0609_id_mef = f0600_id_mef
     left join $df001$.tb0609_modos_efectos_falla as tb_causa_falla
          on tb_causa_falla.f0609_id_mef = f0600_id_mef_causa
where 
  f0600_id_cia = '$001$' and f0600_tipo_docto_padre = 'RP'
  and coalesce(f0600_fecha_ocurrencia_evento, f0600_fr) BETWEEN '$002$' and '$003$'
