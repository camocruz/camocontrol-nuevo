Public Class fm_0300_facturas_compras
    ...
    Private Sub Dg_listado_CellClick(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dg_listado.CellClick
        ...
        tx_id_mov_inv.Text = dg_listado.CurrentRow.Cells("dgocell_id_mov_inv").Value
        'tx_id_oc.Text = dg_listado.CurrentRow.Cells("dgocell_id_oc").Value
        ...
        Select Case nombre_columna
            ...
            Case "dgocell_DocEntrada"
                ...
                Dim csql As String = "select * from " & database.obtener_esquema & ".tb0310_documentos_movimientos_inventarios" _
                                     & " where f0310_id_documento = '" & dg_listado.CurrentCell.Value & "'"
                ...
                If otb_docto.Rows.Count = 0 Then
                    ...
                    Exit Sub
                End If
                cl_utilidades_gestion_compras.mostrar_documento_movimiento_inventario(dg_listado.CurrentCell.Value,
                                                                                      vg_usuario_autoriza,
                                                                                      vg_id_cia,
                                                                                      "N", "S", "D")
    ...
    Private Sub Llenar_items_solicitados()
        dg_listado.Rows.Clear()
        csql = comunes.suministrar_valor_variable_configuracion("ST-0300-49", vg_id_cia)
        csql = csql.Replace("$df001$", database.obtener_esquema)
        csql = csql.Replace("$001$", id_factura_compras)
        ...
        For Each orow As DataRow In otb_items_programados.Rows
            ...
            ocosto += orow("costo_total")
            subtotal += orow("f0309_entrada") * orow("f0305_costo_unitario_planificado")
    ...
    Private Sub Agregar_fila_items(ByVal orow As DataRow)
        ...
        otextgrid = New DataGridViewTextBoxCell With {
            .Value = orow.Item("f0309_id_mov_item").ToString
        }
        ...
        If orow.Item("f0309_id_documento").ToString = "" Then
            otextgrid.Value = "PENDIENTE!!!"
        Else
            otextgrid.Value = orow.Item("f0309_id_documento").ToString
        End If
        'otextgrid.MaxInputLength = 100
        'Agrega columna al objeto fila
        orowgrid.Cells.Add(otextgrid)
        ...
        'crea columna 9
        otextgrid = New DataGridViewTextBoxCell With {
            .Value = orow.Item("f0305_costo_unitario_planificado") / (1 - orow.Item("f0305_descuento"))
        }
        'otextgrid.MaxInputLength = 100
        'Agrega columna al objeto fila
        orowgrid.Cells.Add(otextgrid)
    ...
    Private Sub Bt_add_item_Click(sender As System.Object, e As System.EventArgs) Handles bt_add_item.Click
        ...
        End If
        Cargar_elemento_existente()
        Asignar_items_recibidos_a_factura(2)
    End Sub
    ...
    Private Sub Bt_eliminar_item_Click(sender As System.Object, e As System.EventArgs) Handles bt_eliminar_item.Click
        If tx_id_mov_inv.Text.ToString.Trim = "" Then
            Exit Sub
        End If
        If tx_id_factura.Text.Trim = "" Then
            Exit Sub
        ...
        Cargar_elemento_existente()
        If oaprovada = "S" Then
            MsgBox("La factura ya esta aprovada, no puede modificarse.", MsgBoxStyle.Information, "Info")
            Exit Sub
        End If
        Dim otb_info_item As DataTable
        csql = "select *" _
            & " from " & database.obtener_esquema & ".tb0309_items_movimientos" _
            & " where f0309_id_mov_item = '" & tx_id_mov_inv.Text.Trim & "'" _
            & " and f0309_id_factura_costo = '" & tx_id_factura.Text.Trim & "'"
        otb_info_item = cl_utilidades_datatables.cargar_informacion_postgres(csql)
        verror_requisitos = "N"
        Validar_factura_Aprovada()
        'Valida que exista el registro
        ...
        End If
        'Actualizo el item
        Actualizar_item_solicitado("N", tx_id_mov_inv.Text.Trim)
        If verror = "N" Then
        ...
        End If
    End Sub
    Private Sub Actualizar_item_solicitado(vincular As String, ByVal id_mov_item As Integer)
        'Instancia la conexión que estará vigente para todas las operaciones CRUD
        oconn_form = database.obtener_conexion()
        'actualizacion parametrizada
        csql = "update " + database.obtener_esquema + ".tb0309_items_movimientos set "
        csql += "f0309_id_factura_costo = @f0309_id_factura_costo,"
        csql += "f0309_fm = @f0309_fm,"
        csql += "f0309_usuario_modificar = @f0309_usuario_modificar"
        csql += " where f0309_id_mov_item = '" & id_mov_item & "'"
        ...
        ocmd.Parameters.Add("@f0309_usuario_modificar", NpgsqlDbType.Varchar).Value = vg_usuario_autoriza
        ocmd.Parameters.Add("@f0309_fm", NpgsqlDbType.Timestamp).Value = ofecha
        verror = "N"
        Try
            'Compila el comando en la Base de datos.
            'ocmd.Prepare()
    ...
    Private Sub Abrir_info_solicitud_compra(ByVal id_sc As String)
    ...
    Private Sub Bt_cargar_items_sc_Click(sender As Object, e As EventArgs) Handles bt_cargar_items_sc.Click
        Asignar_items_recibidos_a_factura(1)
        tx_sol_compra.Text = ""
    End Sub
    Private Sub Asignar_items_recibidos_a_factura(ByVal tipo As Integer)
        Cargar_elemento_existente()
        If oaprovada = "S" Then
            MsgBox("La recepcion ya esta aprovada, no puede modificarse.", MsgBoxStyle.Information, "Info")
            Exit Sub
        End If
        'todos los items de una solicitud
        csql = "select * from camocontrol.fnc_300_04_asignar_items_recepcionados_a_factura(tipo, codigo, 'tercero', idfactura)"
        csql = Replace(csql, "tercero", tx_id_tercero.Text.Trim)
        csql = Replace(csql, "idfactura", tx_id_factura.Text.Trim)
        If tipo = 1 Then
            If tx_sol_compra.Text.Trim = "" Then
                Exit Sub
            End If
            csql = Replace(csql, "tipo", tipo)
            csql = Replace(csql, "codigo", tx_sol_compra.Text.Trim)
        End If
        'Un item de una solictud
        If tipo = 2 Then
            If tx_id_item_sc.Text.Trim = "" Then
                Exit Sub
            End If
            csql = Replace(csql, "tipo", tipo)
            csql = Replace(csql, "codigo", tx_id_item_sc.Text.Trim)
        End If
        'Todos los items registrados en una OC
        If tipo = 3 Then
            If tx_id_oc.Text.Trim = "" Then
                Exit Sub
            End If
            csql = Replace(csql, "tipo", tipo)
            csql = Replace(csql, "codigo", tx_id_oc.Text.Trim)
        End If
    ...
    Private Sub Bt_listado_items_pend_Click(sender As Object, e As EventArgs) Handles bt_listado_items_pend.Click
        ...
        If tx_id_tercero.Text.Trim <> "" Then
            Dim otb_pendientes As DataTable
            csql = comunes.suministrar_valor_variable_configuracion("ST-0300-50", vg_id_cia)
            csql = csql.Replace("$df001$", database.obtener_esquema)
            csql = csql.Replace("--f0319_id_tercero = '' and", "f0319_id_tercero = '" & tx_id_tercero.Text & "' and ")
            otb_pendientes = cl_utilidades_datatables.cargar_informacion_postgres(csql)
            otb_tablas_array = cl_utilidades_datatables.visualizar_datos_visor("", vg_id_cia, vg_usuario_autoriza,
                                                          "Items Solicitados para Comprar Sin Recibir", {}, otb_pendientes,
        ...
        For Each orow As DataRow In otb_items_selected.Rows
            ...
            tx_id_mov_inv.Text = orow("id_mov_item")
            Adicionar_items_factura(orow("id_mov_item"))
        Next
        tx_id_item_sc.Text = ""
        Llenar_items_solicitados()
        tx_id_item_sc.Focus()
    ...
    Private Sub Bt_listado_oc_Click(sender As Object, e As EventArgs) Handles bt_listado_oc.Click
        Cargar_elemento_existente()
        Asignar_items_recibidos_a_factura(3)
        tx_id_oc.Text = ""
    End Sub
